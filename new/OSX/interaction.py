...
class Interaction():
  ...
  def move_mouse(self, x, y):
    ...
    if self.left_button_pressed: # Drag!
      Interaction.execute_event(CGEventCreateMouseEvent(None, kCGEventLeftMouseDragged, (self.mouse_x, self.mouse_y), 0))
    else: # Just move
      Interaction.execute_event(CGEventCreateMouseEvent(None, kCGEventMouseMoved, (self.mouse_x, self.mouse_y), 0))
  ...
  def click_down(self):
    Interaction.execute_event(CGEventCreateMouseEvent(None, kCGEventLeftMouseDown, (self.mouse_x, self.mouse_y), 0))
    self.left_button_pressed = True

  def click_up(self):
    Interaction.execute_event(CGEventCreateMouseEvent(None, kCGEventLeftMouseUp, (self.mouse_x, self.mouse_y), 0))
    self.left_button_pressed = False
